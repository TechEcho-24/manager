    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-col gap-2">
          <h1 className="text-3xl font-black tracking-tight text-foreground">Master Command Center</h1>
          <p className="text-sm font-bold text-muted-foreground opacity-40 flex items-center gap-2">
            <ShieldCheck className="h-4 w-4 text-emerald-500" /> Administrative Access Verified
          </p>
        </div>
        <button 
          onClick={() => setIsAdding(true)}
          className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-3 rounded-2xl font-black text-xs tracking-widest shadow-xl shadow-indigo-600/20 active:scale-95 transition-all"
        >
          ...
          <Card key={stat.title} className="border-border/40 bg-card/60 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden group hover:border-primary/40 transition-all">
            <CardContent className="p-7">
              ...
              <div className="mt-5">
                ...
                <p className="text-[10px] font-black text-muted-foreground/50 tracking-[0.2em] mt-1.5">{stat.title}</p>
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <Card className="lg:col-span-2 border-border/40 bg-card/40 backdrop-blur-3xl shadow-2xl rounded-[2.5rem] overflow-hidden">
          <div className="p-8 border-b border-border/10 flex items-center justify-between">
            <h3 className="font-black text-sm tracking-[0.3em] opacity-80">Subscriber Clusters</h3>
          </div>
          <div className="overflow-x-auto">
             <table className="w-full text-[13px] text-left">
                <thead className="bg-muted/30 text-[10px] font-black tracking-widest opacity-30">
                   <tr>
                      ...
                      <tr key={client._id} className="hover:bg-primary/[0.02] transition-colors group">
                         <td className="px-8 py-6">
                            <div className="flex flex-col">
                               <div className="font-black text-foreground text-sm tracking-tight">{client.name}</div>
                   ...
                   {clients.length === 0 && (
                      <tr>
                         <td colSpan={4} className="p-20 text-center flex flex-col items-center gap-4">
                            ...
                            <p className="text-[10px] font-black tracking-[0.5em] text-muted-foreground/30">No Active Clusters Detected</p>
                         </td>
                      </tr>
                   )}
                </tbody>
        ...
        <div className="space-y-6">
           <Card className="border-border/40 bg-indigo-600/5 backdrop-blur-3xl shadow-2xl rounded-[3rem] p-10 flex flex-col items-center justify-center text-center gap-8 group overflow-hidden relative border-t border-t-white/10">
             ...
             <div>
               <h3 className="font-black text-2xl tracking-tighter italic">Neural Pulse</h3>
               <p className="text-[10px] font-bold text-muted-foreground/60 tracking-widest mt-3 ">Optimal signal strength detected across all client instances.</p>
             </div>
             <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
                <div className="w-[99%] h-full bg-indigo-600 shadow-[0_0_20px_rgba(79,70,229,1)]" />
             </div>
             <span className="text-[9px] font-black text-indigo-500 tracking-[0.5em] animate-pulse">99.9% Up-Time</span>
           </Card>
        </div>
      </div>
         ...
         <div className="fixed inset-0 z-[999] flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
            <Card className="w-full max-w-md border-border/40 bg-card shadow-2xl rounded-[2.5rem] overflow-hidden p-8 animate-in zoom-in-95 duration-300">
               <div className="flex items-center justify-between mb-8">
                  <h3 className="text-xl font-black tracking-tight">Onboard New Node</h3>
               ...
               <form onSubmit={handleAddClient} className="space-y-6">
                  <div className="space-y-4">
                     <div>
                        <label className="text-[10px] font-black text-muted-foreground ml-1">Client Name</label>
                     ...
                     <div>
                        <label className="text-[10px] font-black text-muted-foreground ml-1">Email Address</label>
                     ...
                     <div className="grid grid-cols-2 gap-4">
                        <div>
                           <label className="text-[10px] font-black text-muted-foreground ml-1">Revenue (INR)</label>
                        ...
                        <div>
                           <label className="text-[10px] font-black text-muted-foreground ml-1">Tier</label>
                  ...
                  <button type="submit" className="w-full h-14 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl font-black text-xs tracking-[0.2em] shadow-xl shadow-indigo-600/30 transition-all flex items-center justify-center gap-3">
