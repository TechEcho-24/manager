  ...
  return (
    <section className="py-20 md:py-32 px-4 md:px-6">
       <div className="max-w-4xl mx-auto">
          <h2 className="text-3xl md:text-6xl font-black tracking-tighter text-white italic mb-20 text-center">Security Protocols.</h2>
          <div className="space-y-4">
             {[
                ...
                <div key={i} className="reveal group p-6 sm:p-10 rounded-[2rem] sm:rounded-[3rem] bg-white/[0.02] border border-white/5 hover:border-white/20 transition-all" style={{ animationDelay: `${i * 100}ms` }}>
                   <h4 className="text-sm font-black text-white mb-4 group-hover:text-orange-400 transition-colors tracking-widest">{f.q}</h4>
                   <p className="text-xs font-bold text-indigo-100/50 tracking-widest">{f.a}</p>
                </div>
             ))}
          </div>
       </div>
