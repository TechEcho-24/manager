             ...
             {featureItems.map((f, i) => (
                <div key={i} className="reveal group p-10 rounded-[3rem] bg-white/[0.02] border border-white/5 hover:border-orange-500/30 transition-all duration-500 relative overflow-hidden" style={{ animationDelay: `${i * 100}ms` }}>
                   <div className="relative z-10">
                      ...
                      <h3 className="text-xl font-black text-white uppercase mb-4 tracking-tight">{f.title}</h3>
                      <p className="text-sm font-bold text-indigo-100/70 uppercase leading-loose tracking-widest drop-shadow-sm">{f.desc}</p>
                   </div>
